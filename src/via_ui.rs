...
use crate::via_studio_ui;
use crate::via_vial_ui;
use gtk4::gio;
use gtk4::glib;
use gtk4::prelude::*;
use gtk4::{
...
pub fn build_via_page(parent: &impl IsA<Window>, status: Rc<dyn Fn(String)>) -> ViaPage {
    ...
    let studio_tab = via_studio_ui::build_studio_tab(Rc::clone(&get_vid_pid), Rc::clone(&status));
    hub.append_page(&studio_tab.page, Some(&Label::new(Some("Studio"))));
    let sync_studio = studio_tab.sync;

    let vial_tabs = via_vial_ui::build_vial_tabs(Rc::clone(&get_vid_pid), Rc::clone(&status));
    hub.append_page(
