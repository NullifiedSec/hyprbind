...
use crate::dialog;
use crate::waybar;
use crate::waybar_model::{ModuleOrigin, WaybarModel, Zone};
use crate::waybar_modules::{self, FieldKind};
use crate::waybar_style::{self, StyleTokens};
use crate::waybar_themes;
use gtk4::prelude::*;
use gtk4::{
...
fn rebuild_preview(preview: &GtkBox, state: &Rc<RefCell<StudioState>>) {
    clear_box(preview);
    let st = state.borrow();
    let tokens = waybar_style::extract_tokens(&st.model.style_css);
...
fn rebuild_layout_tab(
    ...
    // Add module controls
    let addables = waybar_modules::addable_modules();
...
fn rebuild_modules_tab(
    ...
    let cfg = state
        ...
        .module_config(&name)
        .cloned()
        .unwrap_or_else(|| waybar_modules::default_config_for(&name));

    let schema = waybar_modules::schema_for(&name);
...
fn current_workspace_colors(state: &Rc<RefCell<StudioState>>) -> waybar_style::WorkspaceStateColors {
    waybar_style::extract_workspace_colors(&state.borrow().model.style_css)
}

fn save_workspace_colors(
    state: &Rc<RefCell<StudioState>>,
    colors: &waybar_style::WorkspaceStateColors,
    status: &Rc<dyn Fn(String)>,
) {
    {
        let mut st = state.borrow_mut();
        st.model.style_css = waybar_style::apply_workspace_colors(&st.model.style_css, colors);
    }
    let model = state.borrow().model.clone();
    match waybar::apply(&model) {
...
fn build_workspaces_icons_panel(
    ...
    let cfg = state
        ...
        .cloned()
        .unwrap_or_else(|| waybar_modules::default_config_for("hyprland/workspaces"));
            ...
            edit.connect_clicked(move |_| {
                ...
                let initial = {
                    let cfg = state
                        ...
                        .cloned()
                        .unwrap_or_else(|| {
                            waybar_modules::default_config_for("hyprland/workspaces")
                        });
    ...
    let reset_colors = Button::builder().label("Reset color defaults").build();
    {
        ...
        reset_colors.connect_clicked(move |_| {
            let colors = waybar_style::WorkspaceStateColors::defaults();
            save_workspace_colors(&state, &colors, &status);
            call_refresh(&refresh_ui);
        });
    }
...
fn build_field_row(
    module: &str,
    field: &waybar_modules::ModuleField,
    cfg: &Value,
    readonly: bool,
    state: &Rc<RefCell<StudioState>>,
    refresh_ui: &Rc<RefCell<Option<Rc<dyn Fn()>>>>,
...
fn rebuild_style_tab(
    ...
    clear_box(body);
    let tokens = waybar_style::extract_tokens(&state.borrow().model.style_css);
    ...
    for &(key, title, hint) in FIELDS {
        ...
        edit.connect_clicked(move |_| {
            let initial = token_get(
                &waybar_style::extract_tokens(&state.borrow().model.style_css),
                key,
            );
            let state = Rc::clone(&state);
            let status = Rc::clone(&status);
            ...
            open_text_edit_dialog(
                ...
                move |value| {
                    {
                        let mut st = state.borrow_mut();
                        let mut tokens = waybar_style::extract_tokens(&st.model.style_css);
                        token_set(&mut tokens, key, value);
                        st.model.style_css =
                            waybar_style::apply_tokens(&st.model.style_css, &tokens);
                    }
                    status(format!(
...
fn rebuild_themes_tab(
    ...
    for theme in waybar_themes::all_themes() {
        let row = GtkBox::builder()
            .orientation(Orientation::Horizontal)
            .spacing(12)
            .css_classes(["hyprbinds-settings-row"])
        ...
        let btn = Button::builder().label("Use theme").build();
        {
            ...
            btn.connect_clicked(move |_| {
                let mut st = state.borrow_mut();
                if let Some(css) = waybar_themes::apply_theme_to_css(&st.model.style_css, id) {
