...
use crate::dialog;
use crate::experimental::starship;
use crate::experimental::starship::model::{ShellStatus, StarshipModel};
use crate::experimental::starship::options::{
...
fn rebuild_options_tab(
    ...
    let doc = match super::options::parse_document(&toml_text) {
        Ok(d) => d,
        Err(e) => {
            body.append(
                &Label::builder()
...
fn option_row(
    ...
    let unset = matches!(
        super::options::get_option(doc, opt),
        OptionValue::Unset
    );
    if unset {
        col.append(&dialog::row_meta(&format!(
            "Using default: {}",
            super::options::display_default(opt)
        )));
    }
    ...
    match opt.kind {
        OptionKind::Bool => {
            let check = CheckButton::builder()
                ...
                .active(super::options::effective_bool(doc, opt))
                .build();
        ...
        OptionKind::String => {
            let entry = Entry::builder()
                .text(super::options::effective_string(doc, opt))
                .hexpand(true)
                .width_chars(28)
                .build();
        ...
        OptionKind::Integer { min, max } => {
            ...
            spin.set_value(super::options::effective_integer(doc, opt) as f64);
...
fn write_option(
    ...
    let toml_text = state.borrow().model.toml_text.clone();
    match super::options::apply_option_to_toml(&toml_text, opt, value) {
        Ok(next) => {
