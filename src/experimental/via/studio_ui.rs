...
use crate::dialog;
use crate::experimental::via::{self, LightingSnapshot, PaintBackend, ViaDefinition};
use crate::experimental::via::studio::{
...
pub fn build_studio_tab(get_vid_pid: VidPidFn, status: StatusFn) -> StudioTab {
    ...
    let refresh_presets_dd = {
        ...
        Rc::new(move || {
            let names = super::studio::list_presets().unwrap_or_default();
            ...
            if live_switch.is_active() {
                if let Some((vid, pid)) = get_vid_pid() {
                    ...
                    if backend.is_available() {
                        let _ = via::enter_paint_mode(vid, pid, backend, brightness);
                        match super::studio::push_frame(vid, pid, backend, &frame, &order) {
    ...
    let schedule_playback: Rc<RefCell<Option<Rc<dyn Fn()>>>> = Rc::new(RefCell::new(None));
    {
        ...
        *schedule_slot.borrow_mut() = Some(Rc::new(move || {
            ...
            if let Err(e) = super::studio::push_frame(vid, pid, backend, &frame, &order) {
    ...
    fill_btn.connect_clicked({
        ...
        move |_| {
            ...
            apply_pattern(super::studio::pattern_fill(&keys, brush_color()));
        }
    });
    clear_btn.connect_clicked({
        ...
        move |_| {
            let keys = state.borrow().layout_order.clone();
            apply_pattern(super::studio::pattern_clear(&keys));
        }
    });
    checker_btn.connect_clicked({
        ...
        move |_| {
            ...
            let a = brush_color();
            let b = Rgb::BLACK;
            apply_pattern(super::studio::pattern_checkerboard(&keys, a, b));
        }
    });
    gradient_btn.connect_clicked({
        ...
        move |_| {
            let keys = state.borrow().layout_order.clone();
            apply_pattern(super::studio::pattern_gradient_h(
                &keys,
                sat_scale.value().round() as u8,
                val_scale.value().round() as u8,
            ));
    ...
    rainbow_btn.connect_clicked({
        ...
        move |_| {
            let keys = state.borrow().layout_order.clone();
            apply_pattern(super::studio::pattern_rainbow_rows(
                &keys,
                sat_scale.value().round() as u8,
                val_scale.value().round() as u8,
            ));
    ...
    save_preset_btn.connect_clicked({
        ...
        move |_| {
            ...
            st.anim.name = name;
            st.anim.ensure_frame();
            match super::studio::save_preset(&st.anim) {
                Ok(path) => {
    ...
    load_preset_btn.connect_clicked({
        ...
        move |_| {
            ...
            match super::studio::load_preset(&name) {
                Ok(anim) => {
    ...
    delete_preset_btn.connect_clicked({
        ...
        move |_| {
            ...
            match super::studio::delete_preset(&name) {
                Ok(()) => {
